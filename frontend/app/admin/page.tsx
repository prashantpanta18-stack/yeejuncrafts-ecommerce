...
const dummyProductImages = [
  "https://images.unsplash.com/photo-1515083818318-62024cc5da83?q=80&w=800&auto=format&fit=crop",
  ...
  "https://images.unsplash.com/photo-1620023473177-3e198642751f?q=80&w=800&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1515562141207-7a8efd3e33f0?q=80&w=800&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1610041321360-192e22c06180?q=80&w=800&auto=format&fit=crop",
];
...
const FALLBACK_CATEGORY = dummyProductImages[0];

// 🚨 ULTIMATE ROOT FIX: SafeImage Component
const SafeImage = ({
  src,
  alt,
  className,
...
// Helper to safely extract background images without crashing
const getSafeBgImage = (url: any, fallback: string) => {
...
// Safe Array Logic
const getValidArray = (arr: any, fallback: any[]) => {
...
export default function HomePage() {
  // 🚨 FIXED: Initialize state with DEFAULT_DATA so it loads instantly without any spinner!
  const [settings, setSettings] = useState<any>(DEFAULT_DATA);
  const [currentSlide, setCurrentSlide] = useState(0);

  useEffect(() => {
    // Fetch data in the background silently
    fetch("http://localhost:8080/home-settings", { cache: "no-store" })
      .then((res) => res.json())
      .then((data) => {
        if (data && !data.error) {
          // Merge fetched data with default data to ensure nothing is missing
          setSettings({ ...DEFAULT_DATA, ...data });
        }
      })
      .catch((err) => console.error("Silent background fetch error:", err));
  }, []);

  // Applying Safeguards to ensure no crashes
  const safeHeroSlides = getValidArray(
    settings.heroSlides,
    DEFAULT_DATA.heroSlides,
  );
  const safeCategories = getValidArray(
    settings.categories,
    DEFAULT_DATA.categories,
  );
  const safeStoryMedia = getSafeBgImage(
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % safeHeroSlides.length);
    }, 5000);
  ...
  const SectionHeader = ({
    ...
    <div className="flex justify-between items-end mb-10 border-b border-stone-200 pb-4">
      <div>
        ...
        <div
          style={{ backgroundColor: settings.heroSettings.btnColor }}
  ...
  return (
    <div className="font-sans w-full bg-white">
      ...
      <section className="relative w-full h-[75vh] md:h-[85vh] bg-stone-900 flex items-center justify-center overflow-hidden">
        {safeHeroSlides.map((slide: any, idx: number) => {
          const bgSrc = getSafeBgImage(slide.image, FALLBACK_HERO);
          ...
          return (
            ...
            >
              ...
              <div
                className="absolute inset-0 bg-gradient-to-r from-stone-900/90 via-stone-900/40 to-transparent"
                style={{ opacity: settings.heroSettings.overlayOpacity }}
              ></div>

              <div className="relative z-10 text-left px-6 md:px-16 lg:px-24 w-full h-full flex flex-col justify-end items-start pb-32 md:pb-40">
                <div className="max-w-2xl">
                  <h1
                    style={{ color: settings.heroSettings.textColor }}
                    className="text-4xl md:text-6xl lg:text-7xl font-serif font-extrabold mb-6 leading-tight drop-shadow-lg"
                  >
                    {slide.title || "Authentic Masterpieces"}
                  </h1>
                  <p
                    style={{ color: settings.heroSettings.textColor }}
                    className="text-lg md:text-xl mb-10 max-w-xl font-light opacity-90 drop-shadow-md"
                  >
                    {slide.subtitle || "Discover the soul of the Himalayas."}
                  </p>
                  <Link
                    href={slide.btnLink || "#"}
                    style={{ backgroundColor: settings.heroSettings.btnColor }}
                  ...
                  >
                    {slide.btnText || "Explore"} <ArrowRight size={18} />
                  </Link>
                </div>
              </div>
            </div>
        ...
        <div className="absolute bottom-10 left-6 md:left-16 lg:left-24 flex gap-3 z-20">
          {safeHeroSlides.map((_: any, i: number) => (
            <button
              key={i}
      ...
      <section
        style={{
          backgroundColor: settings.trustSettings.bgColor,
          color: settings.trustSettings.textColor,
        }}
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center md:divide-x divide-stone-200/50">
            {settings.trustBadges.map((badge: any, i: number) => (
              ...
              >
                <div
                  style={{ color: settings.heroSettings.btnColor }}
                  className="mb-4"
                >
          ...
          <div className="flex overflow-x-auto gap-6 pb-8 snap-x hide-scrollbar -mx-4 px-4 lg:mx-0 lg:px-0">
            {safeCategories.map((cat: any, i: number) => (
              <Link
                key={i}
                href={cat.link || "/"}
                className={`group relative h-[380px] w-[75vw] sm:w-[calc(50%-12px)] md:w-[calc(33.33%-16px)] lg:w-[calc(25%-18px)] flex-shrink-0 snap-start overflow-hidden bg-stone-900 ${settings.categorySettings.shape}`}
              >
                <SafeImage
                  src={cat.image}
                  fallback={FALLBACK_CATEGORY}
                ...
                <div className="absolute bottom-0 left-0 p-6 w-full text-center flex flex-col items-center justify-end h-full">
                  ...
                  <span
                    className={`text-xs font-bold uppercase tracking-widest transition-all ${settings.categorySettings.btnStyle === "filled" ? "bg-white text-stone-900 px-6 py-3 rounded-full group-hover:bg-orange-500 group-hover:text-white shadow-lg" : "text-white border-b-2 border-orange-500 pb-1 group-hover:text-orange-500"}`}
                  >
                    Explore
                  </span>
                </div>
      ...
      {/* 4. NEW ARRIVALS */}
      {settings.newArrivalsConfig.show && (
        <section className="py-20 bg-stone-50">
          <div className="max-w-7xl mx-auto px-4">
            ...
            />
            {renderScrollableProducts(settings.newArrivalsConfig.limit)}
          </div>
        </section>
      )}

      {/* 5. FEATURED COLLECTION */}
      {settings.featuredConfig.show && (
        <section className="py-20 bg-white border-t border-stone-100">
          <div className="max-w-7xl mx-auto px-4">
            ...
            />
            {renderScrollableProducts(settings.featuredConfig.limit)}
          </div>
        </section>
      )}
          ...
          <div className="w-full lg:w-2/5">
            <div className="w-full bg-[#292524] border border-stone-700/50 p-8 md:p-12 lg:p-14 rounded-3xl shadow-2xl">
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-serif font-extrabold mb-6 leading-tight text-white">
                {settings.storyConfig?.title ||
                  "Preserving Ancient Craftsmanship"}
              </h2>
              <p className="text-stone-300 text-base md:text-lg mb-10 leading-relaxed font-light">
                {settings.storyConfig?.text1 ||
                  "Every piece is a piece of history."}
              </p>
              <div>
                <Link
                  href={settings.storyConfig?.btnLink || "#"}
                  style={{ backgroundColor: settings.heroSettings.btnColor }}
                ...
                >
                  {settings.storyConfig?.btnText || "Discover"}{" "}
                  <ArrowRight size={18} />
                </Link>
              </div>
            </div>
      ...
      {/* 7. THE MASTERPIECES */}
      {settings.masterpieceConfig.show && (
        <section className="py-24 bg-stone-100">
          <div className="max-w-7xl mx-auto px-4">
            ...
            />
            <div
              className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-${settings.masterpieceConfig.columns} gap-8`}
            >
              {[...Array(settings.masterpieceConfig.limit)].map((_, i) => (
                <div
                  key={i}
        ...
        <section className="py-20 bg-white border-t border-stone-200">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="flex overflow-x-auto gap-8 pb-8 snap-x hide-scrollbar -mx-4 px-4 lg:mx-0 lg:px-0">
              {[...Array(settings.blogConfig.limit)].map((_, i) => (
                <Link
